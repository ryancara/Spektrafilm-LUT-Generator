...
def make_opts(args: argparse.Namespace, state: dict[str, Any]) -> SimpleNamespace:
    ...
    if output is None:
        ...
        sample_label = {"medium": "CLF36", "large": "CLF64", "huge": "CLF121"}.get(args.size, f"CLF-{args.size}")
        input_label = {"aces-ap0": "AP0-linear", "rec2020": "Rec2020-linear", "srgb": "sRGB", "prophoto": "ProPhoto", "state": "state-input"}.get(args.input_mode, args.input_mode)
        if args.format == "cube":
            suffix = f"{input_label}__{sample_label}__OCIO-CUBE{args.cube_size}"
        else:
            suffix = f"{input_label}__{sample_label}"
        output = outdir / f"{safe_filename(film_value)}__{safe_filename(str(paper_value))}__{safe_filename(suffix)}.{ext}"
...
def get_args() -> argparse.Namespace:
    ...
    p.add_argument("-s", "--size", choices=["medium", "large", "huge"], default="medium", help="CLF sampling quality: medium=36³ standard, large=64³ high, huge=121³ extreme/slow.")
    p.add_argument("--cube-size", type=int, default=64, help="3D cube size for OCIO-baked CUBE output. Recommended: 64.")
    ...
    p.add_argument("--input-gain", type=float, default=0.0, help="Generator-only input gain, written as CDL in the reference CLF before optional OCIO CUBE bake")
